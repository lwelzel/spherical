...
def config_to_dict(config) -> Dict[str, Any]:
    """Plain, JSON/YAML-serialisable dictionary of a composite reduction config.

    ``dataclasses.asdict`` alone leaves ``Path`` objects and ``set`` values (the
    ``steps.force`` cascade) in the result; this converts them so the dictionary
    can be written as provenance next to the products and read back with
...
def config_from_dict(data: Dict[str, Any], instrument: str = "ifs"):
    """Rebuild an ``IFSReductionConfig`` / ``IRDISReductionConfig`` from :func:`config_to_dict` output.

    Sub-config keys map onto the dataclass fields of the composite; unknown keys
    raise ``TypeError`` so a stale dictionary cannot silently drop settings.
    ``steps.force`` given as a list is restored to a ``set``.
    """
    cls = IFSReductionConfig if instrument.lower() == "ifs" else IRDISReductionConfig
    config = cls()
    for name, value in data.items():
        if not hasattr(config, name):
            raise TypeError(f"{cls.__name__} has no field {name!r}")
        current = getattr(config, name)
        if isinstance(value, dict) and hasattr(current, "merge"):
            if name == "steps" and isinstance(value.get("force"), list):
                value = {**value, "force": set(value["force"])}
            if name == "directories":
                setattr(config, name, DirectoryConfig(**value))
            else:
                setattr(config, name, current.merge(**value))
        else:
